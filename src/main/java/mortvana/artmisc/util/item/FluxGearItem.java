...
import mortvana.artmisc.common.ArtificersMiscellanea;
...
import mortvana.artmisc.util.RegistrationWrapper;
import mortvana.artmisc.util.helpers.TextureHelper;
	...
	*/
	...
	public ItemStack addItem(int metadata, ItemEntry entry, boolean register) {
		if (itemList.contains(metadata)) {
			ArtificersMiscellanea.logger.warn("Someone registered a meta-item in an item from " + modName + ", with a metadata of " + metadata + ", called " + entry.getName() + "! Don't do this! Skipping Entry!");
			return null;
		} else {
    ...
    public void registerIcons(IIconRegister register) {
		ArtificersMiscellanea.logger.info("Registering Icons");
		ItemEntry entry;
